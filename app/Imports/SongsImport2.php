...
class SongsImport2 implements ToCollection, WithHeadingRow, WithChunkReading
{
    public $clientId ;
    public $clientSecret;
    ...
    $topTracksEndpoint = "https://api.spotify.com/v1/artists/$artistId/top-tracks?country=US";
    // Make the API request to fetch artist's top tracks
    $topTracksResponse = $client->request('GET', $topTracksEndpoint, [
        'headers' => $headers,
    ]);

    // Get the response body for artist's top tracks
    $topTracksData = json_decode($topTracksResponse->getBody(), true);

    // Check if any top tracks are found
    if (!empty($topTracksData['tracks'])) {
        // Loop through the top tracks and import them
        foreach ($topTracksData['tracks'] as $topTrackData) {
            // Extract relevant information for each top track
            $topTrackId = $topTrackData['id'];
            $topTrackName = $topTrackData['name'];
            // ... (similar to how you extracted information for the searched track)

            // Store the data in the database for each top track (similar to how you stored the data for the searched track)
            $topSong = Song::firstOrCreate([
                'track_id' => $topTrackId,
            ], [
                'song_genres' => implode(',', $artistData['genres']),
                'artist' => $artistName,
                'artist_id' => $artistId,
                'youtube_id' => $topTrackData['name'],
                'karaokevid' => $topTrackData['name'],
                'album' => $album,
                'spotify_uri' => $topTrackData['uri'],
                'preview_url' => $topTrackData['preview_url'],
                'duration_ms' => $topTrackData['duration_ms'],
                'popularity' => $topTrackData['popularity'],
                'image_url' => $imageUrl,
                'lyrics' => $lyrics, // Note: $lyrics is not defined in the code you provided. You'll need to fetch lyrics separately for each top song.
                'track_name' => $topTrackName,
                'created_at' => now(),
                'updated_at' => now(),
            ]);
        }
